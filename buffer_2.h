...
/// Interface
////////////////////////////////////////////////////////////
static void InitBufferStack2(BufferStack2* buffer_stack, BufferStackInfo2* info)
{
...
static void InitBuffer2(Buffer2* buffer, BufferStack2* buffer_stack, VkDeviceSize size)
{
    ...
    buffer->offset     = buffer_stack->index + aligned_size;
    buffer->size       = aligned_size;
    buffer->index      = 0;
    buffer->mapped_mem = buffer_stack->mapped_mem == NULL ? NULL : buffer_stack->mapped_mem + buffer->offset;
}

static void WriteHostBuffer2(Buffer2* buffer, void* data, VkDeviceSize data_size)
{
...
static void AppendHostBuffer2(Buffer2* buffer, void* data, VkDeviceSize data_size)
{
    CTK_ASSERT(buffer->mapped_mem != NULL);
...
static void WriteDeviceBufferCmd2(Buffer2* buffer, Buffer2* staging_buffer, VkDeviceSize offset = 0, VkDeviceSize size = 0)
{
    if (size == 0)
    {
...
static void AppendDeviceBufferCmd2(Buffer2* buffer, Buffer2* staging_buffer, VkDeviceSize offset = 0,
                                   VkDeviceSize size = 0)
{
    if (size == 0)
    {
...
static void Clear2(Buffer2* buffer)
{
    buffer->index = 0;
}
