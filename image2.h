...
{
    ImageTypes types;
    ImageStack stacks[VK_MAX_MEMORY_TYPES];
};
...
static ImageTypeHnd PushImageType(ImageMemory* image_memory, ImageType* image_type, VkDeviceSize size)
{
    ...
    // Create image with image-type data, then get image-type's mem-type-index from image's memory requirements.
    ...
    image_types->mem_type_indexes[image_type_hnd] = GetMemoryTypeIndex(&mem_requirements, &image_type->mem_properties);
}
